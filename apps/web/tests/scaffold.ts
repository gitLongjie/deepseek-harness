...
// package's complete TS project into this graph.
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  try {
    ...
    // The shipped login gate blocks first-run pages behind the account
    // server's sign-in. Scenarios exercise signed-in product flows against
    // keyless fixtures, so the scaffold seeds the gate's own local session
    // through one head script on every served index — the same storage state
    // a real sign-in leaves behind, with the account server kept out of the
    // lane entirely.
    ctx.on('webserver/index-inject', (table) => {
      const session = JSON.stringify({ account: 'e2e', avatar: null, apiKey: 'dsh-e2e-seeded' })
      table.push({
        kind: 'script',
        placement: 'head',
        text: `localStorage.setItem('dsh.login.session', ${JSON.stringify(session)})`,
      })
    })
...
export function realizeSeedFixture(scaffold: WebScaffold, fixtureText: string, id: string): string {
  // Substituted paths land inside JSON string literals, so they must carry the
  // JSON escaping (Windows separators otherwise break the header parse).
  const jsonEscape = (value: string): string => JSON.stringify(value).slice(1, -1)
  const realized = fixtureText
    .split('{{sessionId}}').join(id)
    .split('{{cwd}}').join(jsonEscape(scaffold.workspaceCwd))
  const fixtureCwd = (JSON.parse(realized.split('\n', 1)[0]!) as { cwd?: string }).cwd
  return fixtureCwd === undefined
    ? realized
    : realized.split(jsonEscape(fixtureCwd)).join(jsonEscape(scaffold.workspaceCwd))
}
