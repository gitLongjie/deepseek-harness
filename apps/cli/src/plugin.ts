...
/**
 * Reconcile the profile's layer list through the shared app-boot helper; the
 * CLI owns the stderr sink and the running installation's resolution anchor.
 */
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  reconcileProfileBundles(NAME, before, profileDir, INSTALL_ANCHOR, (message) => {
