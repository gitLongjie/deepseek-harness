...
仓库根目录的 [`oem.config.json`](../oem.config.json) 统一提供打包后的产品名称、品牌图标、登录端点、桌面更新 URL，以及空面板五个时间段的中英文问候语。`brandIcon` 指向 `apps/web/public` 下一个根相对 `.ico` 文件；桌面开发与打包流程会把该源文件复制到原生应用和托盘图标位置，Vite 则将其用于 document favicon、Web App Manifest、标题栏、启动页、登录页及客户端品牌组件。`updateUrl` 是 electron-updater 读取 `latest.yml` 等频道元数据并下载安装包的 HTTPS 基础目录。替换公共图标、在文件名变化时更新其 URL、设置 OEM 更新目录，然后执行普通构建。`scripts/oem-config.ts` 会在 Vite 或动态 client bundle 编译前拒绝缺失、多余、空值、非本地图标、HTTP 或 HTTPS 之外的登录端点、非 HTTPS 更新 URL，或不完整的问候语字段。单次构建可用显式的 `DSH_CLIENT_BRAND_NAME`、`DSH_CLIENT_BRAND_ICON`、`DSH_CLIENT_LOGIN_URL`、`DSH_CLIENT_GREETINGS_ZH`、`DSH_CLIENT_GREETINGS_EN` 和 `DSH_CLIENT_TITLE` 覆盖文件值；图标覆盖值也必须指向本地公共 `.ico` 文件，确保原生界面与 Web 界面使用相同图案。
...
`pnpm --filter @deepseek-ai/dsh-desktop run smoke` 会打包桌面应用，在隔离的 user-data 与 DSH-home 目录下启动真实的解包产物，并按其启动自检结果门禁：三个已就绪的 IPC 载体服务、打包前端 index 渲染、ripgrep 实际 spawn、workflow worker 解析，以及 Windows 上的 koffi 原生加载和一次必须以 runner 坏参数失败签名应答的 Node 模式沙箱 runner 运行。打包应用在每次启动时断言同样的资源完整性，并在宿主挂载前以本地化弹窗响亮失败，缺失的运行时资源要么在门禁处失败、要么在启动处失败，而不会以静默错乱的形式发布出去。传入 `--publish` 可在冒烟通过后才执行 GitHub 发布构建；发布 workflow 在三个平台上传前都会运行这道门禁。迭代时可传入 `--skip-build` 对既有的 `dist-electron` 产物重新冒烟。`src/main/desktop/packaged-resources.ts` 是 asar-unpack 清单与启动期断言的单一事实来源；原生模块与 Node 模式加载闭包应维护在该模块中，而不是 `electron-builder.yml`。决策见[打包冒烟门禁 Note](../.agents/notes/implemented/bug-fix/2026-09-03-desktop-packaged-smoke-gate.zh.md)。
