...
The repository-root [`oem.config.json`](../oem.config.json) owns the packaged product name, brand icon, login endpoint, desktop update URL, and five Chinese and English blank-panel greetings. `brandIcon` names one root-relative `.ico` file under `apps/web/public`; desktop development and packaging copy that source into the native application and tray slots, while Vite uses it for the document favicon, Web App Manifest, title bar, boot page, login, and client brand components. `updateUrl` is the HTTPS base directory from which electron-updater reads channel metadata such as `latest.yml` and downloads release artifacts. Replace the public icon, update its URL when its name changes, set the OEM update directory, and run the normal build. `scripts/oem-config.ts` rejects missing, extra, empty, non-local icon, a login endpoint outside HTTP or HTTPS, a non-HTTPS update URL, or incomplete greeting fields before Vite or a dynamic client bundle compiles. Explicit `DSH_CLIENT_BRAND_NAME`, `DSH_CLIENT_BRAND_ICON`, `DSH_CLIENT_LOGIN_URL`, `DSH_CLIENT_GREETINGS_ZH`, `DSH_CLIENT_GREETINGS_EN`, and `DSH_CLIENT_TITLE` values override the file for one build; an icon override must also name a local public `.ico` file so native and Web artwork stay identical.

### Local desktop update rehearsal

On Windows, build an installed baseline and a newer update feed with two increasing semantic versions:

```powershell
pnpm --filter @deepseek-ai/dsh-desktop run test:update:local -- --base-version 1.0.0-local.1 --update-version 1.0.0-local.2
```

The command writes ignored artifacts under `apps/desktop/.release/local-update/`, validates the channel metadata, installer, and blockmap, and serves a range-capable generic feed on `http://127.0.0.1:43119`. It prints the baseline installer path after both builds succeed. The updater derives the metadata channel from the installed version's prerelease identifier, so `1.0.0-local.1` reads `local.yml` rather than `latest.yml`. Keep the command running, install that baseline, and launch the installed application. Startup discovery stays silent and adds an Update button to the title bar. An explicit Help-menu check instead opens a localized result dialog; confirming an available version starts the download, while deferring leaves the title-bar action available. Downloading replaces the button label with percentage progress. When the package is ready, a localized dialog offers an immediate restart or deferral; restart disables the control with installer-start feedback, waits up to five seconds for application resources to close, and then opens the visible native installer. The updater's following quit bypasses the normal cleanup interception so NSIS does not race the old process; a cleanup timeout still permits installation. Deferral preserves the restart action, and repeated install actions are ignored. A failed check exposes a retry action. Stop the feed with Ctrl+C after the rehearsal. The desktop deploy manifest declares runtime-only Service Definition packages and the configured local IM plugin so electron-builder includes the complete shipped plugin tree. The script does not change a package version or publish artifacts. Its private packaging environment embeds an exact local-update test marker, and the installed runtime accepts HTTP only when that marker is present and the URL names `127.0.0.1`, `localhost`, or `[::1]`; ordinary OEM and release builds continue to require an HTTPS update URL. Rebuild and reinstall the baseline after changing this mechanism because an older installer does not contain the marker or corrected dependency set. Pass `--port <port>` to select another loopback port.

### Packaged desktop smoke

`pnpm --filter @deepseek-ai/dsh-desktop run smoke` packages the desktop app, boots the real unpacked artifact under an isolated user-data and DSH-home directory, and gates on its boot self-check: the settled IPC carrier services, the packaged frontend index render, a ripgrep spawn, workflow worker resolution, and on Windows the koffi native load plus a Node-mode sandbox runner run that must answer with the runner's bad-argv failure signature. The packaged app asserts the same resource completeness at every startup and fails loud with a localized dialog before the host mounts, so a missing runtime resource fails the gate or the launch instead of shipping as silent misbehavior. Pass `--publish` to run the GitHub release build only after a passing smoke; the publish workflow runs this gate on all three platforms before uploading. Pass `--skip-build` to re-smoke an existing `dist-electron` artifact while iterating. `src/main/desktop/packaged-resources.ts` is the single source of truth for the asar-unpack manifest and the startup assertions; keep native modules and Node-mode load closures there rather than in `electron-builder.yml`. See the [packaged smoke gate note](../.agents/notes/implemented/bug-fix/2026-09-03-desktop-packaged-smoke-gate.md) for the decision.
