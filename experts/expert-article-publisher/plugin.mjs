/**
 * The @xmanrui/expert-article-publisher mount: syncs the packaged expert
 * directory into the harness home's user preset root, where the agent-presets
 * roster's user root discovers it on its next read (trust: user).
 *
...
export function apply(ctx, config) {
  ...
  if (expertDir === '' || targetDir === '') {
    throw new Error(
      'expert-article-publisher: the mount needs `expertDir` and `targetDir` in its bundle patch config',
    )
  }
