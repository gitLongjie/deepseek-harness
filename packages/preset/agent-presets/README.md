...
- **Health asks what is installed, not what would import** — discovery proves the composition parses in the loader dialect, holds named rows, and that each row it can prove will start names a package present above the harness base or a file that exists; it never imports one, so a package whose own entry file is missing, a plugin that throws on apply, and one waiting forever for a service all still fail at the first session. The harness base is the composition directory's base, except inside a packaged application archive (`app.asar`), where `installedHostBase` anchors it at the archive root — the only tree whose `node_modules` a closed runtime can walk. `disabled` is the one entry field the Loader interpolates, so a row carrying an expression there is left unchecked rather than judged from the file.
