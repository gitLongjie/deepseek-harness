...
preset 目录可以在组装文件旁携带一个 `preset.yml`，其中只保存展示文本：面向每个选择器的 `name`、`description` 与 `order`，以及供市场式选择器分组与过滤的专家卡片字段——部署自有的 `category` id、至多八个 `tags`、至多三条作为建议首条消息的 `quickPrompts`，以及一个简短的 `icon` 字形。该文件绝不携带身份或信任信息（`id` 即目录，`trust` 来自根配置）；任何缺失或格式错误的字段都退化为无元数据，而不会产生一个无法启动的 preset。
