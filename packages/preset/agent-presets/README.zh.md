...
preset 目录可以在组装文件旁携带一份 `preset.yml`，其中只有展示文本：面向所有选择器的 `name`、`description` 与 `order`，以及市场式选择器据以分组和筛选的专家卡片字段——部署自有的 `category` 分类 id、至多八条 `tags` 标签、至多三条 `quickPrompts` 推荐提问、一个短 `icon` 字形。该文件永不携带身份或信任（`id` 来自目录名，`trust` 来自所在根目录），任何缺失或格式错误的字段都降级为无元数据，而不会变成一个无法启动的 preset。
...
复制会解引用符号链接以保证自包含，把目录树收紧为仅属主可用（文件 `0o600` 并保留属主执行位，目录 `0o700`），并在首次复制时创建根目录。复制出的 `preset.yml` 会被重写：保留来源的描述供作者编辑，丢弃其名称与 roster `order`，从而让名单始终能区分副本与来源；专家卡片字段（`category`、`tags`、`quickPrompts`、`icon`）原样随行，因为它们说明的是这项专长是什么，而不是这个副本是哪一个。删除拒绝随部署提供的 preset，并清除指向刚删除 preset 的用户默认值。
...
- **健康问的是「装没装」，不是「能不能 import」**——发现过程证明组装能以加载器方言解析、由具名行组成，且每一行它能证明会启动的行所引用的包装在 harness 基准之上、或所引用的文件确实存在；它从不 import 任何一个，因此入口文件缺失的包、在 apply 时抛错的插件、以及永远等待某个服务的插件，都仍在第一个会话处失败。harness 基准取组合目录的基址，唯一例外是打包应用归档（`app.asar`）内——`installedHostBase` 会把它锚定到归档根，那是封闭运行时唯一能走查到 `node_modules` 的树。`disabled` 是加载器唯一会插值的条目字段，因此在该字段写了表达式的行会被跳过，而不是仅凭文件下判断。
