...
describe('the persona row', () => {
  it('shadows and interpolates the environment per scope, restoring both defaults on disposal', async () => {
    ...
    try {
      ...
      expect(renderPrompt(assembly)).toBe('You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.\n\nPreset.\n\nUse tools.\n\nWorkspace /local.')
