...
`withFileLock` 跨进程串行化同一文件的写入方，服务于单靠原子提交无法保证安全的读-渲染-提交循环。锁是以 `wx` 创建的同目录 `<filename>.lock`，其中记录持有者的 pid，因此读取方从不参与竞争；等待方按指数退避，超时即失败而非无限阻塞。`EEXIST` 直接表示竞争；只有一次新的 `lstat` 确认锁路径存在时，`EPERM` 才表示竞争，从而兼容 Windows 的独占创建行为，又不掩盖无关的权限故障。竞争者只在锁内记录的 pid 不对应任何存活进程时才移除锁——即持有者在释放步骤前已死亡；其余一切竞争（存活的所有者、被复用的 pid、无法判读的内容）都只会等待到期并失败，绝不删除无法证明为孤儿的锁。
...
- **孤儿锁恢复信任锁内记录的 pid，复用即失败方向**——记录的 pid 不对应任何存活进程的锁会被自动清除；被操作系统复用过的 pid（或本协议无法判读的内容）会读作存活，这类孤儿仍需操作者恢复。文件存续时间本身永远不是废弃的证据。
