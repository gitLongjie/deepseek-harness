...
In an Electron host the runner spawn carries `ELECTRON_RUN_AS_NODE: '1'`, so the runner runs as plain Node instead of a second application instance; each spawn builds the target environment on the parent, so the switch never reaches a target. A caller abort that fires before the runner reports a target outcome rejects `done` as a provider failure and terminates the runner — a wedged startup never outlives the caller's deadline.
