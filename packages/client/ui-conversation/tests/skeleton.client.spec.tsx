...
function mount(
  ...
  const props: ConversationRootProps = {
    ...
    useKnowledgeView: select => select({ open: false }),
    useExpertView: select => select({ open: false }),
    useInput,
    inputActions,
    renderSlot,
    renderSlotChain,
...
describe('Hero chrome', () => {
  it('renders the English hero greeting through the hero locale seat', () => {
    const renderSlot = vi.fn<HeroShellProps['renderSlot']>(() => null)
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} renderSlot={renderSlot} />)
    expect(view.getByText(/Explore the unexplored/u)).toBeTruthy()
    ...
    // No fish fallback: an empty seat is the fork's no-occupant posture.
    expect(renderSlot.mock.calls[0]?.[2]?.fallback).toBeUndefined()
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.getByText(/探索未至之境/u)).toBeTruthy()
  ...
  it('keeps a rejected first prompt engaging instead of returning to the Hero', () => {
    ...
    expect(b.view.queryByText(/探索未至之境/u)).toBeNull()
  })
