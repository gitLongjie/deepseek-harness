...
export function apply(ctx: Context, config: Config = Config({})): void {
  ...
  ctx.effect(() => ctx.locale.register(NS, configuredGreetingCopy(
    zh,
    en,
  ...
  // The knowledge browser's view source, resolved per bind under the rule
  // documented with `ABSENT_KNOWLEDGE_VIEW`: the ui-knowledge-base plugin is
  // an optional mount whose apply may run after this one, so the service is
  // looked up at bind time, never captured here — an eager capture would
  // freeze the absent source and the browser would never react to a base
  // being opened. Once the service exists its store identity is stable, so
