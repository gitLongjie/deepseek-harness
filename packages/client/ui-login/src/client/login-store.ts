...
export class LoginStore {
  ...
  constructor(
    ...
    private readonly api: Pick<ConnectionHandle['api'], 'llm' | 'settings'>,
  ) {}
  ...
  private async syncCatalogFromGateway(apiKey: string): Promise<void> {
    const discovered = await this.api.llm.discoverModels({
      settingsNs: 'llm-deepagens',
      baseURL: `${this.baseUrl()}/v1`,
      apiKey,
    })
    if (!discovered.result.ok) {
      console.warn(`[ui-login] gateway model discovery refused: ${discovered.result.error.code}`)
      return
    }
    const catalogModels = discovered.result.value.models.map(m => ({
      id: m.id,
      name: m.name ?? m.id,
      description: '',
    ...
    if (!described.result.ok) {
      console.warn(`[ui-login] settings describe refused: ${described.result.error.code}`)
      return
    }
    const ns = described.result.value.namespaces.find(view => view.ns === 'llm-deepagens')
