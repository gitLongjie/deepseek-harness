...
- **Preview, not editing.** The viewers provide no file editing or shared search interface; a directory address fails with `not-regular-file`. Unknown extensions and the Host-extractable documents (`.doc`, `.docx`, `.odt`) use the plain-text reader — extraction failures surface on the body as `no-converter` or `conversion-failed` lines — and remain subject to its UTF-8/NUL checks.
