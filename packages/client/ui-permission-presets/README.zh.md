---
description: "Web 与 Desktop 共用的浏览器 GUI 权限预设：通用设置中的默认行与切换当前会话的 /permission 选择器。"
...
本包为服务端 Web 与 Desktop renderer 共用的浏览器 GUI 提供两种生命周期的权限预设：通用设置中的一行选择之后创建会话所用的默认值，但不会切换当前会话。挂在宿主 `/permission` 命令上的选择器通过一张扁平预设列表切换当前会话，并标记 active 值。三个内置预设标签跟随客户端语言；其他 kebab-case 名称渲染为 Title Case，其他宿主定义名称原样显示。选择完全权限时，该行或选择器写入前必须先显式确认风险。两个表面读取同一份宿主计算的投影、经同一条路径写入，因此推送的投影帧是两者共同跟随的唯一确认。
