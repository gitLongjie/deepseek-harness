...
- **MewoLogo 标记是内嵌位图**：深度Works 的 favicon 以 base64 PNG data URI 内嵌在 `img` 元素中；它保持固定的品牌墨色，不像字形图标那样跟随 `currentColor`。
