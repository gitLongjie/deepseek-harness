...
node 半侧逐包增量扫描。每次 `internal/plugin` 发出都会把该 fiber 的 entry 名标脏；一个微任务 flush 会把每个脏名与当前 loader 条目对账，激活 pass 播种同一脏集合并同步 flush，因此首次扫描与稳态共用同一实现。每次读取启动图也会播种当前 Loader 行，并在注入 HTML 前 flush；这会捕获注册表观察生命周期事件之前创建的条目。在打包应用中，裸条目从导入它们的安装宿主基准地址解析；相对路径与 file 条目保留所属 tree 的 base URL。包元数据按 Loader specifier 与解析基准地址缓存至重启，解析出的 manifest 包名作为浏览器模块身份。若不同的 active Loader source 解析到同一包名，组合会失败；移除冲突来源后，剩余来源无需重启 fiber 即可接替。bundle 内容变更只能通过 `rebuilt()`（HMR 钩子）进入图。
