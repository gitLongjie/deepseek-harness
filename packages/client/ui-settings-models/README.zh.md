...
每个模型行还可以声明输入类型：文本始终启用，视觉模型可以启用图片。pi-ai 编辑器写入 `input`，DeepSeek 编辑器写入 `inputModalities`；现有的请求时图片能力检查会消费该字段。
...
- **卡片上只有 API 密钥与精选折叠字段可编辑**：手写编辑器以 schema 通用字段覆盖换取了 mockup 布局。重试策略、超时、DeepSeek 模型说明及其他进阶字段仍留在 `settings.yaml` 中；编辑器未展示的现有模型字段会予以保留。路由卡片渲染哪个家族由适配器自行声明（其 configurable-provider 条目上的 `editorFamily`）；适配器未声明家族的路由渲染进阶字段提示，而不是猜测一张卡片。
