...
describe('locale apply', () => {
  ...
  // opens on the product default (zh); each test that reads other localized
  // copy stages its locale explicitly via setLocale/Host preference.
  ...
  it('provides the service with base + settings dictionaries and registers the row (declaration before or after apply)', async () => {
    ...
    // A fresh service opens on the product default (zh); read it directly.
  ...
  it('loads and refreshes the explicit Host preference after nonblocking activation', async () => {
    ...
    // Preference must differ from the provisional locale (the product default,
    // zh), or clearing it below would be unobservable.
    ...
    // Re-applying en after the clear is an explicit pick that must persist.
    b.setHostPreference('en')
    b.events.emit('settings/document-updated', [LOCALE_SETTINGS_NAMESPACE, 0])
    await vi.waitFor(() => { expect(locale.getLocale().active).toBe('en') })
