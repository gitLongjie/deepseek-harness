        ...
        presets: [
          {
            ...
            name: '标准模式', description: '完整的编码 agent。',
          },
          {
            id: 'geo-optimizer', trust: 'system' as const, isDefault: false,
            name: 'GEO 优化专家', description: '可见度诊断与报价。',
            category: 'marketing', tags: ['GEO', '报价'],
            quickPrompts: ['先诊断可见度'], icon: '🔍',
          },
        ],
        authorable: true,
      },
...
describe('ui-expert browser plugin', () => {
  ...
  it('reads the roster through the page inject and hires stage-then-start', async () => {
    const remote = stub()
    const b = await bench(remote)
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    ...
    expect(presets.map(preset => preset.id)).toEqual(['standard', 'geo-optimizer'])
  ...
  it('renders the empty market for an unavailable agentPresets namespace', async () => {
    ...
    const page = b.slots.entries('conversation.expert.browser')[0]
    const injected = (page?.inject as unknown as () => { load: () => Promise<{ presets: unknown[] }> })()

    // A deployment composing no presets falls back to mock data so the page
    // renders a populated demo instead of an empty state.
    const result = await injected.load()
