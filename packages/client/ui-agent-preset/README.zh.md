...
使用本包可以为新的 Web GUI 会话选择 agent preset、在会话标题中查看当前 preset，并在设置中管理可用 preset。Agent 模式选择器默认显示；设置可以隐藏它，而不会改变运行中或历史会话。preset 在会话创建时即固定，因此更改选择或默认值只影响此后创建的会话。如果部署未提供任何 preset，这些控件保持隐藏，每个会话都使用宿主组装。 发布专家卡片元数据的 preset 属于专家页的库存，不出现在这些列表中。
...
与设置和对话包一起挂载本插件；管理分区随后显示一个默认开启的可见性开关。关闭期间，新建会话 chip 不出现，宿主会依据部署默认值（随附 Web bundle 中为 `standard`）组装未指名会话。开启时会恢复已保存的用户默认值；尚未保存时则使用部署默认值；该默认值会同时带到当前空白任务上。chip 中的选择本身只为下一个空白会话暂存一次。再次关闭选择器会以同样方式把当前空白任务带回部署默认值，并丢弃尚未使用的暂存选择；已开始及历史会话的标签、组装与已记录历史均保持不变。芯片始终写明下一个对话将运行的组装，包括选择器无法提供的那一种：专家中心聘入空白会话的对话会显示该专家的名称，并用提示说明它从何而来，而不是只显示标识符。之后的新会话流程接管空白会话时，会把它恢复到新会话应有的组装——但只在这些界面不可能选择过当前组装时才恢复，因此芯片或设置页上选定的模式会保留，而被聘用留下组装的对话不会变成下一个新会话的组装。
...
工作区的空白会话就是它的「新会话」占位会话，并被多个流程共享：聘用会把专家组装进它，而下一个新会话流程又会接管同一个会话。`bindPlaceholderAdoption` 就是本包恢复「新建会话会解析出的那个组装」的地方——但只在本包自己的界面不可能选择过当前组装时才恢复，因此芯片或设置页选定的模式保持不动，而聘用专家的对话不会被带进下一个新会话；已暂存的选择会立即返回，因为把会话切换到它是 applier 的职责。名称解析读取整个健康名单而不是芯片自身的选项，因此运行专家标记预设的会话由该预设命名，而菜单仍然只提供模式。
