...
名单携带自指的 `cordis` preset 时，一张虚线添加卡会暂存它并开启新会话——分区关闭设置面板，新建会话 chip 自己的应用器负责组装工作区流程产出的空白会话。同一暂存动作也以 `uiAgentPreset` 根服务（`stageNextSessionPreset`）的形式发布，因此其他界面——专家中心的聘用动作——可以为下一个会话暂存选择，而无须触及本包的 slot；该服务转发给当前拥有座位流的会话绑定，在没有绑定时按空操作处理。
