  ...
  override async run(spec: ShellExecSpec): Promise<ShellRunResult> {
    ...
    try {
      ({ result, spawnRequested } = await this.runArgv(spec, async (signal) => {
        ...
        confined = prepared
        // The runner program is the direct child of this spawn; carry its
        // required environment entries (Node mode inside an Electron host)
        // by publishing them onto the spec the executor spawns with.
        spec.env = this.withRunnerEnvironment(spec, confined).env
