...
- `ctx.workspaceRegistry.delete(id)` — removes only the Workspace registration, its durable order entry, and its session account, and moves the member Sessions into the global archive set in the same committed write so they never resurface as Ungrouped. Unknown ids return `false`; a removed record returns `true`. The directory, user files, and persisted session logs are never touched, and unarchiving restores the archived Sessions. A table-write failure restores the prior order and published entity.
