...
- `ctx.workspaceRegistry.delete(id)`：只移除 Workspace 注册记录、对应的持久顺序条目及会话归属记录，并在同一次提交的写入中把成员会话移入全局归档集合，使其不会重新出现在 Ungrouped 中。未知 id 返回 `false`，成功移除记录则返回 `true`。目录、用户文件和持久化会话日志绝不受影响，取消归档即可恢复这些会话。表写入失败时会恢复原顺序和此前发布的实体。
