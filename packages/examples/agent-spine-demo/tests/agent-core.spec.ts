...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    ...
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by MeowWork. Unless explicitly instructed otherwise, respond in the same language the user uses.')
