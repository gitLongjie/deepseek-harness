...
async function collectActivePluginPackages(
  ...
  if (sessionId !== undefined && typeof ctx.get === 'function' && ctx.get('agentPresets') !== undefined) {
...
export function apply(ctx: Context, config: Config): void {
  ...
  // The packed-image test invokes apply with a Loader-metadata-only context
  // whose service verbs are absent; resolve the registry only when the context
  // is a live Cordis context (get present) rather than a fake carrier.
  const resolver = new PackageIdentityResolver(
    hostBaseUrl,
    typeof ctx.get === 'function' ? ctx.get('pluginPackages') : undefined,
  )
