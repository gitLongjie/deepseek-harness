...
import { canOpenNativePath, nativeFileManager, openNativeDirectory, openNativePath, revealNativePath } from '@deepseek-ai/dsh-native-command'
...
export class SessionController extends TypertRemoteService {
  ...
  async openWorkspacePath(
    ...
    try {
      ...
      else if (request.action === 'directory') await this.openDirectory(request.path, signal)
