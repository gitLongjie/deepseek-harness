...
describe('agent loop', () => {
  ...
  it('renders harness identity, then the persona, then tool guidance — with {{variables}} resolved', async () => {
    ...
    expect(systemOf(request)).toBe('You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.\n\nYou are a test agent on mock.\n\nUse the noop tool wisely.')
  ...
  it('resolves {{cwd}} from the agent session workspace (factory create with meta.cwd)', async () => {
    ...
    expect(systemOf(adapter.requests[0])).toBe('You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.\n\nWorking in /work/space.')
  })
  ...
  it('keeps a later nonempty prompt ahead of history after an initially empty assembly', async () => {
    ...
    try {
      ...
      await secondIdle
      expect(adapter.requests).toHaveLength(2)
      expect(systemOf(adapter.requests[1])).toBe('You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.')
