...
describe('agent loop', () => {
  ...
  it('renders harness identity, then the persona, then tool guidance — with {{variables}} resolved', async () => {
    ...
    expect(request!.system).toBe('You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.\n\nYou are a test agent on mock.\n\nUse the noop tool wisely.')
  ...
  it('resolves {{cwd}} from the agent session workspace (factory create with meta.cwd)', async () => {
    ...
    expect(adapter.requests[0]!.system).toBe('You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.\n\nWorking in /work/space.')
  })
