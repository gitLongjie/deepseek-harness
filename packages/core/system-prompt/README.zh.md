...
| 字段 | 默认值 | 含义 |
|---|---|---|
| `includeHarnessIdentity` | `true` | 是否包含顺序为 −1000 的 first-party 固定开场白 `You are an AI agent powered by MeowWork. Unless explicitly instructed otherwise, respond in the same language the user uses.`。仅当兼容性部署拥有完整系统提示词时设为 false。 |
