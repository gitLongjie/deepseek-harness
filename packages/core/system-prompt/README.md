...
|---|---|---|
| `includeHarnessIdentity` | `true` | Include the fixed `You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.` first-party opener at order −1000. Set false only when a compatibility deployment owns the complete system prompt. |
...
```markdown
You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.
```
