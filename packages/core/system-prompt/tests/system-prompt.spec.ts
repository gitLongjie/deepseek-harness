...
const IDENTITY = 'You are an AI agent powered by 深度Work. Unless explicitly instructed otherwise, respond in the same language the user uses.'
...
describe('SystemPrompt', () => {
  ...
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      await ctx.plugin(SystemPrompt, { persona: 'You are MiaoO Harness.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are MiaoO Harness.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
