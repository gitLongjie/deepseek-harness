...
- **Console creation flags are unavailable** — children created with `CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` die during DLL initialization with `STATUS_DLL_INIT_FAILED` (`0xC0000142`). The shared process layer instead requests `STARTF_USESHOWWINDOW` with `SW_HIDE`, preserving the inherited console and pipe-based stdio routing while hiding an initial console window.
