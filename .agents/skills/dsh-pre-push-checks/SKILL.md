...
- **Desktop packaging configuration, the packaged resource manifest, or the desktop main-process boot path:** run `pnpm --filter @deepseek-ai/dsh-desktop run smoke` before sharing or publishing an installer; it boots the real unpacked artifact and gates on its boot self-check (`--skip-build` re-smokes an existing `dist-electron` artifact only when the packaging inputs are unchanged).
