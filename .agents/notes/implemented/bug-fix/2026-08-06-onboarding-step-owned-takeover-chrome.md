...
The primitive spec (removed with the whole onboarding mechanism on 2026-08-27) pinned the primitive: body portal around the content, mask/stage class presence, `#root` inert held for exactly the mount lifetime, and the no-`#root` composition. `packages/client/ui-settings-general/tests/settings-root.client.spec.tsx` pinned the inverted shell contract: no takeover chrome and no inert while a mounted step renders nothing. The assembled regression spec in the web onboarding e2e file (removed with the mechanism) reloaded a configured world while every `settings.describe` response was held open at the browser's network boundary — widening the steps' deciding window from loopback-invisible to hundreds of milliseconds, which is what kept the assertions non-vacuous — and an 8 ms in-page sampler proved the takeover chrome never mounted and `#root` never turned inert. The step specs (`ui-settings-general`, `ui-settings-models`) passed unchanged — the mask selector and geometry pins survived because the stylesheet moved verbatim.
