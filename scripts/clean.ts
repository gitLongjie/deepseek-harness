  ...
  private buildOutputDirectories(): string[] {
    ...
    while (pending.length > 0) {
      ...
      if (parsed.options.outDir !== undefined) {
        ...
        // The desktop shell is a private Electron product: its `dist` output
        // is neither a lib build nor a workspace-wide clean target — the
        // packaging scripts own its lifecycle.
        const desktopShellOutput = typesDirectory === join(this.root, 'apps/desktop/dist')
        const outputDirectory = basename(typesDirectory) === 'types'
          ? dirname(typesDirectory)
          : typesDirectory === nativeEntryOutput || desktopShellOutput
            ? typesDirectory
            : undefined
